...
def dirty_lawyer_CPI():
    ...
    while True:
        choice = input(
            "Now that you spent that money on the PI lets put him to work. The Private investigator found little evidence to get the lawyer to work for you, but it works. Now will you get the private investigator to help on your case? (A) Help or (B) Dont Help: ")
        if choice == "a":
...
def innocent_lawyer_fake_photo_friend():
    ...
    while True:
        choice = input(
            "The Lawyer finds an error with the Judge. The judge threatened the jury!! What now? (A)Expose the Judge or (B)Threaten the Judge: ")
        if choice == "a":
            print(
...
def innocent_lawyer_stripper():
    ...
    while True:
        choice = input(
            "Lucky for you shes pretty cheap? Now what do you want her to do?(A) Tell the lawyer of your plans or (B) Make stripper confront him and threaten him: ")
        if choice == "a":
...
def blackmail_innocent_lawyer():
    ...
    while True:
        choice = input(
            "Wow! Anything it takes to clear your name huh? how you gonna do it?(A) Hire a stripper to lie or (B) Fake a photo: ")
        if choice == "a":
            ...
            print(
                "So were gonna fake a photo huh? We'll pay someone to slip something in his drink\nand take a picture with her the threaten to show his wife.")
            finesse += 10
            s()
...
def legal_blackmail_prove():
    ...
    while True:
        choice = input(
            "Wow so your arent as evil as the rest. Now lets see what path you choose between these two *evil laugh*(A) Prove or (B) blackmail: ")
        if choice == "a":
...
def final():
    ...
    while True:
        choice = input(
            " yess after hard work 4 days straight with only 2 hours of sleep you made it but you are in 3rd floor of the building (A) just jump (B) try to slowly go down by holding the bricks ")
        if choice == "a":
...
def room2():
    ...
    while True:
        choice = input(
            " good news you have way there yess its been a month and you have been digging at nigth and sleeping at noon but you want to speed up? (A) keep it as it is (B)speed up: ")
        if choice == "b":
...
def checkup():
    ...
    while True:
        choice = input(
            " you got the scissors in your pants but there is another problem that you need to face.. security check up!!!!!!!!!! (A)act normal (B) tell them you are in hurry:  ")
        if choice == "a":
...
def emergancy():
    ...
    while True:
        choice = input(
            "yesss you made it to the emergancy room. now you have to distract the doctor somehow.  (A) tell doctor that security is waiting (B) get a deal with doctor: ")
        if choice == "a":
            print(
                " 'doctor' what are u talking about why is he calling me 'i dont know DR but he told me to tell you that he is waiting ' ")
...
def hand():
    ...
    while True:
        choice = input(
            "you want to see the doctor and you want to go there by breaking your hand are you still want to do that  (A) giving up its all over (B) yes im breaking my hand: ")
        if choice == "b":
...
def pretending():
    ...
    while True:
        choice = input(
            " its hard very hard to go to the doctor because securities will check if you are really hurt but its your choice (A) pretend that you are sick (B) stay and make a better plan: ")
        if choice == "a":
            print(
                "WOW now you will get checked by securities  / " " 'security' hey so you are sick come closer whispiring TRY SOMETHING BETTER and he punched you in a face ")
            warning -= 101
            s()
            time.sleep(1)
            print("=============GAMEOVER==========")
...
def doctor_office():
    ...
    while True:
        choice = input(
            " to ask for help you need to be hurt you have two choices (A) break your hands by punching the wall or (B) pretend to be sick:")
        if choice == "a":
            ...
            print(
                " doctor will know that you are pretending, and before doctor security will check you as well but you can try ")
            pretending()
            warning -= 50
            s()
            break
...
def Distract_security():
    ...
    while True:
        choice = input(
            "you chose to distract security guards now you have two choices (A) start a fight between people or (B) get help from your friend : ")
        if choice == "a":
            print(
                " yoo iron he said your pussy cat " '"Iron said what who"' '"here this guy"' '"come over here"' "fight started")
            conscious -= 20
            s()
            kitchen()
            break
...
def sneak_into():
    ...
    while True:
        choice = input(
            "you chose to sneak into the kitchen to get a knife you have two choices (A) go to the kitchen during lunch (B) try to get a deal with chief ")
        if choice == "a":
...
def long_dig():
    ...
    while True:
        choice = input(
            "you have been digging for weeks but you only digged 5 inches do you want to change things around? (A) keep digging (B) try to get a knife: ")
        if choice == "a":
...
def short_dig():
    ...
    while True:
        choice = input(
            " you have been digging for days but no results and your spoon is so weak what you want to do (A) keep digging with patients (B) try to get a knife :")
        if choice == "a":
...
def spoon():
    ...
    while True:
        choice = input(
            "you got the spoon with you and you are in your room ready to dig but where remember which side of the room takes you outside of the building (A) dig behind bed (B) dig behind toilet: ")
        if choice == "a":
...
def dig_your_way_out():
    ...
    while True:
        ...
        b = "dig using a knife"
        choice = input(
            "you chose to dig your way out now you have two choices (A)do you want to use spoon or (B) do you want to use knife: ")
        if choice == "a":
...
def Lisa_janitor():
    ...
    while True:
        ...
        if choice == "a":
            ...
            print(
                "After more questioning Luke found himself with no solid evidence. Luke decides to leave the janitor alone after not getting much from him. Choose another option!")
            break
        else:
            print("Not an option")
...
def getting_pin():
    ...
    time.sleep(1.5)
    print(
        "Word around the office is that Lisa was having an affair with a fellow coworker. She visited the boss only on Tuesdays and Thursdays after 5pm when everybody at the office was gone.")
    print(
        "Lisa is a very stubborn person so convincing her to give us the pin number is going to be a struggle. " + character_name + " visits Lisa at her home in the upper class neighborhood of Orange County, California.")
    print(
        "Upon approaching the gate a car was leaving that " + character_name + " recognized. An idea came forth! Lisa was suprised to see me and acted nervous...")
    print(
        "Hello Lisa! Was that John? *Lisa became even more nervous* Look... I know you are covering up for him so give me the pin number or else you'll be going down with him! With a loss of words Lisa hands " + character_name + " a paper with the pin number.")
    print(
        "While " + character_name + " and the forensics team were on their way back to the office to enter the pin, their car starts to make a flapping sound. They got a flat tire... Fixing the flat tire would take 15 minutes to do but they would have to go to the store to get some tools. Calling the office to send a worker to pick them up would take 45 minutes but will be free since its on the office. The sooner we get back to the office, the closer " + character_name + " is to proving their innocence. What will it be? (A) Fix the flat tire or (B) Call the office for a pickup?")
    a = "Fix the flat tire"
...
def fix_flat():
    ...
    time.sleep(1.5)
    print(
        "After struggling with fixing the car a oncoming truck swerves off the road and hits you and your team head on.")
    print("=============GAMEOVER==========")
...
def call_office():
    ...
    print(
        "The whole team searches the office up and down for any clues relating to this murder. After searching for a whole hour the team gets frustrated as they have no real leads.")
    print(
        "Just when everyone was about to give up a member of the forensics team trips over a stuck up nail in the hardwood floor. The tea mfinds this very odd and soon realizes this was done on purpose.")
    print(
        "Taking up the floor is theyre next move." + character_name + " calls a buddy and within 30 minutes the hardwood floor is out and there is a trap door! Inside the trap door" + character_name + "finds a note.")
    print(
        "The note reads: meet me at the pier on Saturday to find out the truth. Meeting with this person could be very dangerous as they are potientally the murderer. What will you do? Meet the mysterious person (A) or Ignore the note (B)")
    ...
    while True:
        ...
        if choice == "a":
            ...
            print(
                "The team continues searching and find no other clues. A week later your court date comes and with no solid information you cannot prove your innocence.")
...
def meet_person():
    ...
    time.sleep(1.5)
    print(
        "Two days passes by and Saturday is here. Judgement day is on Wenesday and your running out of time. Time to start making some moves.")
    print(
        "You arrive at the pier at the designated time and you see a man in a black cloak sitting by himself. To your suprise you see the Janitor!!! At this point your fuming and want to beat his a** but part of you wants to keep your cool.")
    ...
    while True:
        ...
        if choice == "a":
            print(
                "You take out your glock and smoke this fool as fast as Thanos snapped his fingers and wiped out half the population. Doing this obviously didnt help your case in the end.")
            print(
                "Judgement days arrives and it turns out that you were found to be framed in the murder of your boss. This didnt matter though as you did commit 1st degree murder...")
...
def keep_cool():
    ...
    time.sleep(1.5)
    print(
        "The janitor starts to give his story. He explains to you that he was the one who purposely cut the cameras out. You have enough information to buid your case and prove your innocence in court but you get this weird feeling in your gut.")
    print(
        "Why whould the killer turn himself in so easily? Is this a cover up for the real murderer? Your court date is coming up fast and you need to make a decision! Bring the janitor in as the killer (A) or find out more information (B)")
    ...
    while True:
        ...
        if choice == "a":
            print(
                "You go to court with the janitor and try to prove your innocence. Even with the audio recording you had secretly taken of the janitor confessing to cutting the cameras off you still dont have enough solid information.")
            print("Despite your best efforts you are still given the death penatly.")
            print("=============GAMEOVER==========")

        elif choice == "b":
            print(
                "You go back to the office and put together small pieces of information to help your case. Judgement day arrives and you walk into the court room with confidence.")
            print(
                "You use the audio recording of the janitor to prove him guilty of being involed with the murder but he isnt the murder. The real murderer was a member of the forensics team you hired! His name is John.")
            print(
                "John was playing a role on the forensics team to try and steer you into thinking it was the janitor. John forgot to cover up his tracks though.")
            print(
                "John didnt realize that he left his phone in the car on the way back to the office." + character_name + "finds this and has all the phone call conversations between John and the janitor and their plan.")
            print(
                "John stumbled onto to floor in disbelief, he though nobody would ever find out. After presenting this information you were found not guilty! The janitor went to jail on a charge of tampering with evidence of a murder.")
            print("The janitor was found guilty and sentened to life in jail without the chance of parole.")
            print(
                "Congratulations! You did it! You go home to your friends and family and live the rest of your life stress free.")
            print("THE END")
            exit()
...
def kill_knock():
    ...
    time.sleep(1.5)
    print(
        "After 10 minutes of talking to Wutang,a nerd " + character_name + " saved from getting violated in the shower, wants to help him. " + character_name + " goes up to Haward to talk about the plans for escape. \n So far Haward built DIY high tech heat gun called a melter wherein the heat gun, can turn its output of energy to lava/ hot air, and fireball so it can melt through iron, metal or even human flash in mere seconds. \n The most unexpected thing happened. Haward didn’t want to escape prison, he likes it here but he will Luke escape which is why Haward bult the device.")
    print(
        "Its 1pm at night in prison. Its drizzing outside, and the guards are feeling sleepy. So this was a perfect opportunity for Luke to escape.")
    print(
        character_name + " sees a guard drunk near a wall facing the other side of " + character_name + "'s cell. He is barely concious.")
    print("So," + character_name + " melts part of the cell bar with his melter and prepares to attack the guard")
    print(
        "Choose whether" + character_name + " should kill the guard by spraying a large amount of the substance of the melter at the guard or sneak behind the guard and choke him unconcious. So kill or knock")
    a = "Kill"
    b = "Knock"
    choice = input(
        "What is option do you choose? (A)Kill the guard by spraying a large amount of the substance of the melter at the guard or (B)sneak behind the guard and choke him unconcious to knock him out. ")
    if choice == "a":
        print(
            "The sound of the skull melting alerts a nearby guard and blood is from the skull is melted with skin is all over the floor. The guard arrives at the scene and you have no way of hiding the messy body so the guard rings the alarm. \n The SWAT team comes in arrests " + character_name)
        print("=============GAMEOVER==========")
        warning -= 100
        conscious -= 100
        stealth -= 100
        ...
        print(
            character_name + "The guard is unconcious. " + character_name + " sees desk on the side of him. You hide the body behind the desk and rush to next place down the hallway.")
        stealth += 80
        finesse += 60
        doorred_doorblue()
    else:
...
def doorred_doorblue():
    ...
    time.sleep(1.5)
    print(
        character_name + " starts speed walking with low sound through the hallway until you see two different doors one colored blue and the other colored red with no labels. ")
    a = "red door"
    b = "blue door"
    while True:
...
def spoondistract_knifedistract():
    ...
    while True:
        ...
        if choice == "a":
            print(
                character_name + " threw a spoon from top of the cabinet on the side of one of the guards. The trejictory of the spoon did land near the guard but ended up sliding towards a oven. The sound of the spoon starled the guards.")
            print(
                "But because the spoon landed near the oven which was a bit farther than what was hoped to have landed. So one of the guards said that he will check it out. While the other one stayed in the same position.")
            print(
            ...
            print(
                character_name + " chose to use the knife and threw it at the guard on the left on his head. In that few seconds while the other guard was very surprised and wasnt sure what to make of the situation.")
            print(
                character_name + " uses that few seconds of opportunity to blast the head off with the 2nd part of the melter gun called called heat blast since the melter would take longer as the guard has a protective helmet.")
            print(
                character_name + " moves past the the bodies and was about to open the door. At that point " + character_name + " notices a small ventilation pathway that leads to the outside world.")
...
def spearing_selfdistraction():
    ...
    while True:
        choice = input(
            "What option do you choose? (A) Spear tackle the guard to the oven and use the melter to attack the other guy. \n (B) Pretend to stop, and let" + character_name + "be cought and then when the guards start to cuff" + character_name + "you bring out your shaving blade and kill both guards: ")
        if choice == "a":
            print(
                character_name + " spears tackles towards the guard facing oven and baaaaamm!! The guards face hits the hot oven with fire spursing out and the guard screams with tremendous pain. \n The other guard hears the scream and without a moments notice booom, the guard fires the shotgun at" + character_name + ". ")
            ...
            print(
                character_name + " uses it self as distraction and gets on the ground and pleas about giving up. So both guard approaches " + character_name + " and gets killed immideately as planned.")
            print(
                character_name + " starts walking past the kichen and sees another door but this time lots of iron bars.")
            print(
                "All that can be heard is the grawling from a distance. He really do want to get out of prison and realizes he does have a powerful weapon so theres nothing to be afraid off.")
            print(
                "So," + character_name + " opens the door and as " + character_name + " approches more there were notcibly lots of red eyes and smell of saliva. The grawling gets louder and baaammm!! \n Out of nowhere a dog attacks " + character_name + " and you shoot the dog. But now more are approaching")
            warning += 20
            conscious += 30
            stealth += 80
            finesse += 50
...
def fight_flee():
    ...
    while True:
        choice = input(
            "What option do you choose? (A)Fight all the dogs in the room or (B)Run away from the room and start going back to the kichen.")
        if choice == "a":
            print(
                character_name + "starts shooting all around the room with the lava melting part of the gun, but the dogs were overwhlming. \n So many kept coming attacking from all direction. After 5 minutes of battle" + character_name + " gets tired and in the moment of tiredness, two huge bull dogs appear and rips " + character_name + " to shreds")
            ...
            print(
                character_name + "starts running for his life but sadly the mistake" + character_name + "made was not hiding the bodies before and now theres a swarm of gaurds and they notice" + character_name + "running.")
            print(
                "The guards got startled and started shooting at" + character_name + "and all those various bullets hit" + character_name + "severely. His body got disesembered by all the bullets and dies immedietly.")
            print("=============GAMEOVER==========")
            warning -= 80
            conscious -= 40
            stealth -= 80
